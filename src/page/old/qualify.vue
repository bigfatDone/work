<template>
  <div class="whole">
    <div class="contains">
      ...
      <div class="main" v-if="this.qualifyList.length">
        <div class="item" v-for="(item, index) in qualifyList" :key="index">
          <div class="item-left">
            ...
            <div class="item-left-category">
              <span>{{item.NAME}}</span>
              <span class="color-green">（检测合格）</span>
              <span v-if="item.ORDER_STATUS == 3" class="binding-tip">(已绑定标签码)</span>
              <span v-if="item.ORDER_STATUS == 2" class="unBinding-tip">(未绑定标签码)</span>
            </div>
      ...
      <div v-if="this.qualifyList.length" class="pagination">
        <el-pagination
  ...
  methods: {
    ...
    getQualifyList(val) {
      ...
      record({
        ...
        if (res.success) {
          this.qualifyList = res.resultList;
    ...
    handlePage(val) {
      ...
      this.getQualifyList(val);
    }
  },
  mounted() {
    this.getQualifyList(1);
  }
};
</script>
